...
- 当前最新验证：`D:\Miniconda\envs\dlm\python.exe -m pytest -q`，结果 `114 passed in 22.84s`；P9-4b 验收脚本确认 diverse8 full matrix 满足 `TotalCertificates = 224`、`ReproducedCertificates = 224`、`CertificateReproductionRate = 100.00%`、`HardFalseIndependent = 0`、`CertifiedFeatureMismatchCount = 0`、`RunFailed = 0`，static filter pre 为 `StaticCandidateRecall = 100.00%`、`StaticFalseNegativeObserved = 0`、`StaticCandidateReduction = 21.43%`。
