...
本清单用于约束 `data/` 目录的保留与清理。原则是：输入保留、final 结果保留、work/旧临时结果可删、可再生成的大体量中间产物不进 Git。
...
3. `*_work` 目录默认可删除。
4. 旧 commit 命名的 P5 目录若已有更新 final 目录替代，可删除。
5. 删除前先确认路径位于 `E:\project\data` 下，不对仓库外路径执行递归删除。
